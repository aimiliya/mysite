...
class RegForm(forms.Form):
    ...
    password = forms.CharField(label='密码', min_length=6,
                               widget=forms.PasswordInput(
                                   attrs={'class': 'form-control',
                                          'placeholder': "请输入密码"}))
    password_again = forms.CharField(label='请再次输入一次密码', min_length=6,
                                     widget=forms.PasswordInput(
                                         attrs={'class': 'form-control',
                                                'placeholder': "请输入密码"}))
...
class BindEmailForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email):
            raise forms.ValidationError('该邮箱已经存在，不能绑定')
        return email
...
class ForgotPasswordForm(forms.Form):
    ...
    verifiction_code = forms.CharField(label='验证码', required=False,
                                       max_length=20, widget=forms.TextInput(
            attrs={'class': 'form-control', 'placeholder': '点击 "发送验证码" 发送到邮箱'}))
    new_password = forms.CharField(label='新的密码', min_length=6,
                                   widget=forms.PasswordInput(
